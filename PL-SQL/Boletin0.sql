...
BEGIN
	...
	END IF;
END;
--Si se da la condicion mostrara por pantalla "cierto", y sino mostrara "falso".
...
END;
--Si se cumple la condicion con dos elementos muestra "cierto", sino "falso".
--En este caso se cumple el primer elemento pero no el segundo, asi que no se cumple.
...
BEGIN
	...
	END CASE;
END;
--Es el mismo caso que el ejercicio anterior pero con otra sintaxis (switch).
...
--La variable i no hace falta que la declare.
...
BEGIN
	...
	END LOOP;
END;
--Muestra por pantalla la variable i dentro de un bucle reverso (cuenta hacia atras).
...
BEGIN
	...
	END LOOP;
END;
--Mientras el numero sea menor o igual a 100 se hara el bucle (mostrar valor y 
--sumar 2 al numero).
