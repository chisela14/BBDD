...
PROCEDURE mostrarCines AS 
	...
	CURSOR C_PELIS(V2_CINE CINE.CINE%TYPE, V_SALA SALA.SALA%TYPE) IS
		...
		WHERE PR.CIP = P.CIP AND PR.CINE = V2_CINE AND PR.SALA = V_SALA;
	NUM_SALAS NUMBER := 0;
	NUM_PELIS NUMBER := 0;
	REC_PELI NUMBER := 0;
BEGIN 
...
DECLARE 
	numSalas NUMBER := 0;
BEGIN 
	SELECT COUNT(SALA) INTO numSalas FROM SALA WHERE CINE = :NEW.CINE;
	DBMS_OUTPUT.PUT_LINE(NUMSALAS);
	IF numSalas >=5 THEN 
