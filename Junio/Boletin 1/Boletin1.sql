...
CREATE OR REPLACE PACKAGE PCK_SALARIOS AS
	v_oficio EMPLEADO.OFICIO%TYPE;
	v_sal EMPLEADO.SALARIO%TYPE;
	v_salOLD EMPLEADO.SALARIO%TYPE;
END;
...
FOR EACH ROW 
BEGIN 
	SELECT :NEW.OFICIO INTO PCK_SALARIOS.v_oficio FROM DUAL;
	SELECT :NEW.SALARIO INTO PCK_SALARIOS.v_sal FROM DUAL;
	SELECT :OLD.SALARIO INTO PCK_SALARIOS.v_salOLD FROM DUAL;
END valores_sueldo;

CREATE OR REPLACE
TRIGGER COMPROBAR_SALARIOS
AFTER INSERT OR UPDATE ON EMPLEADO
DECLARE
	v_sueldoMIN EMPLEADO.SALARIO%TYPE; 
	v_sueldoMAX EMPLEADO.SALARIO%TYPE;
BEGIN
	SELECT MAX(NVL(PCK_SALARIOS.v_salOLD,0)) INTO PCK_SALARIOS.v_sueldoMAX FROM EMPLEADO WHERE OFICIO = PCK_SALARIOS.v_oficio;
	SELECT MIN(NVL(PCK_SALARIOS.v_salOLD,0)) INTO PCK_SALARIOS.v_sueldoMIN FROM EMPLEADO WHERE OFICIO = PCK_SALARIOS.v_oficio;
	DBMS_OUTPUT.PUT_LINE(PCK_SALARIOS.v_sueldoMAX|| PCK_SALARIOS.v_sueldoMIN);
	--NO TENGO PRESIDENTE AÑADIDO, PRUEBO CON DIRECTOR
	IF (PCK_SALARIOS.v_oficio != 'DIRECTOR') THEN
		IF PCK_SALARIOS.v_sal < PCK_SALARIOS.v_sueldoMIN OR PCK_SALARIOS.v_sal > PCK_SALARIOS.v_sueldoMAX THEN
...
UPDATE EMPLEADO SET SALARIO = 400 WHERE EMP_NO = 3;
...
BEFORE INSERT OR DELETE OR UPDATE OF NUM_DEP ON EMPLEADO
FOR EACH ROW
BEGIN
